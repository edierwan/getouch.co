...
- Langfuse: direct external observability and tracing UI on `langfuse.getouch.co`.
...
| Coolify | Infra & Persistence | Installed | https://coolify.getouch.co | Coolify remains the canonical external deployment plane. The portal sidebar now opens the real Coolify UI directly, and legacy portal Coolify routes redirect there. | Coolify PostgreSQL, Redis, Cloudflare Access |
