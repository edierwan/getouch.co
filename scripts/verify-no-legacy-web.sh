...
FORBIDDEN_SERVICE_PATTERN='^(web|portal-web|nextjs|frontend)$'
...
check_local_compose_file() {
  ...
  if grep -Eq '^[[:space:]]{2}(web|portal-web|nextjs|frontend):[[:space:]]*$' "$compose_file"; then
    fail "${compose_file} still defines a retired portal service"
  fi

  if grep -Eq 'container_name:[[:space:]]*(portal-web|portal-web-prod)([[:alnum:]_.-]*)?$' "$compose_file"; then
    fail "${compose_file} still defines a retired portal container name"
  fi
}
...
pass 'local compose files do not define a retired portal service'

if grep -Eq 'portal-web|portal-web-prod' infra/Caddyfile; then
  fail 'infra/Caddyfile still mentions a retired portal upstream'
fi
pass 'local infra/Caddyfile does not mention retired portal upstreams'
...
pass 'local deploy scripts do not reference the retired compose service path'

legacy_containers="$(remote_sh "docker ps -a --format '{{.Names}}' | grep -Ei 'portal-web|portal-web-prod' || true")"
if [[ -n "$legacy_containers" ]]; then
  printf '%s\n' "$legacy_containers"
  fail 'retired portal containers still exist on the host'
fi
pass 'no retired portal containers exist on the host'
...
if grep -Eq "$FORBIDDEN_SERVICE_PATTERN" <<<"$remote_services"; then
  fail 'remote compose.yaml still defines a retired portal service'
fi
...
if [[ -n "$remote_alt_services" ]] && grep -Eq "$FORBIDDEN_SERVICE_PATTERN" <<<"$remote_alt_services"; then
  fail 'remote docker-compose.yml still defines a retired portal service'
fi
pass 'remote compose configs do not define a retired portal service'

project_service_hits="$(remote_sh "docker ps -a --filter label=com.docker.compose.project=getouchco --format '{{.Names}} {{.Label \"com.docker.compose.service\"}}' | grep -E ' (web|portal-web|nextjs|frontend)$' || true")"
if [[ -n "$project_service_hits" ]]; then
  printf '%s\n' "$project_service_hits"
  fail 'active getouchco containers still expose a retired portal service label'
fi
pass 'active getouchco containers do not expose a retired portal service label'
...
if remote_sh "docker exec '$CADDY_CONTAINER' getent hosts portal-web >/dev/null 2>&1"; then
  fail 'caddy still resolves a retired portal alias'
fi
if remote_sh "docker exec '$CADDY_CONTAINER' getent hosts portal-web-prod >/dev/null 2>&1"; then
  fail 'caddy still resolves a retired portal production alias'
fi
pass 'caddy does not resolve retired portal aliases'

active_caddy_hits="$(remote_sh "docker exec '$CADDY_CONTAINER' grep -n -E 'portal-web|portal-web-prod' /etc/caddy/Caddyfile 2>/dev/null || true")"
if [[ -n "$active_caddy_hits" ]]; then
  printf '%s\n' "$active_caddy_hits"
  fail 'active Caddy config still mentions a retired portal upstream'
fi
pass 'active Caddy config does not mention retired portal upstreams'
...
if [[ -n "$remote_script_hits" ]]; then
  printf '%s\n' "$remote_script_hits"
  fail 'active host deploy scripts still reference a retired compose service path'
fi
pass 'active host deploy scripts do not reference the retired compose service path'
